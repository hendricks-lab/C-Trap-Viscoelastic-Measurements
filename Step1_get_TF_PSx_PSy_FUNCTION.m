...
%%% updated as compared to the xQUAD and xAOD channel. Similarily, 
%%% the power spectrum graph is also directly from the raw signal, and
%%% thus is from a slighlty larger sample than the FRF (when cutting
%%% is enabled). This should not matter as the corner frequency should 
%%% not be affected.

%Ora's Changes:
%10.2020 - Commented out a bunch of figures because they were annoying
...
%05.2021 - Changed NFFT calculation from NFFT_generator_loic_v4_constant_windows 
%          output to half max NFFT. This works much better for short signals (<5 min)
%          bc it doesn't give a coherence of 1.0, and also seems to work well for 
%          the long signals. However, it will throw errors if your signal is too 
%          short (if you have frequencies longer than your signal length). Maybe
%          one day I'll fix that.
%08.2021-  Fixed the cutpeaks routine in case there are a different number of peaks
%          per frequency, which apparently was thought to be impossible when the 
%          code was written but is in fact possible. Who knew.


function [H,Fexc,C,FP_x,PYm_x,FP_y,PYm_y,Pyy,Pyy2,fsamp,length_used_out,sumQUAD_mean,Npeaks_out,all_loc_out,K_int_out,frequency_range_tolerance,NFFT,number_windows,data_lost,Noverlap]= Step1_get_TF_PSx_PSy_FUNCTION(N_channels,kpd,AODx_input,QUADx_input,QUADy_input,nfftcoef,hannw,plot_range,cut_peaks_data,mean_threshold,cut_threshold,mov_wind_s,plot_cut,Fexc_in,fsamp_in,fileID,manualcut,manualcut_data_end)
    
fsamp = fsamp_in;
   
    if N_channels == 8
        %%%% Reading from text file (needs to be in the current folder)
        rawdata = textscan(fileID, '%f %f %f %f %f %f %f %f', 'HeaderLines', 0);
    elseif N_channels == 4
    ...
    xAOD = (rawdata{AODx_input}).*1000; % [nm] converted from um
    ...
    [FP_y,PYm_y] = power_spectrum_Adam_Loic_FUNCTION([ctime,yQUAD]',Nwindow,Noverlap,Nfft,Nint,fsamp_in);
     ...
     for k = 1:length(Fexc_in) % looping through the frequencies of excitation % 9 - 0.813
        k       
        if cut_peaks_data == 1 %currently cut peaks is not active
