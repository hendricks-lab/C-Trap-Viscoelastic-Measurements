...
clc, clear all,close all %close all %, warning off
%{
addpath('C:\Users\oratm\OneDrive - McGill University\SURE_scripts\loic_codes-master\adams-method') %path where some functions are stored
%}
...
datdir = 'C:\Users\derek\OneDrive - McGill University\Bien 471\shared file 471\Working with modulus and visc fitting'; % defining the path to where the transferfunction.mat files are
addpath(datdir)

%set filename to multiharmonic measurement
flist{1} = {'26.04.23-12.22-TF-TF 1um sep bead 2.mat'}; %name of file to load
...
alphaLB = 0.01; %lower bound on the alpha value (-Ora)
alphaUB = 1; %upper bound on the alpha value (-Ora)

% set initial guess
beta_guess = 1000; % manual input
gammar_guess = 5; %1; %1.68; % manual input, this means the drag coeff. of the cytoplasm is "ig.GammaR" times the one of water
alpha_guess = 0.5; %0.75; % manual input
ktrap_guess = 0.2; %0.05; %0.1; % manual input
kcyt0_guess = 0.01; % manual input
kcyt1_guess = 0.001; %0.002; % manual input

cohthreshh = 0.8; %set coherence threshold for points used in the fit
minDatPoints = 0.75; %min percentage (if you multiplied by 100%) of selected data points from magnitude response required for fitting (-Ora)
...
lb=[100     0.1*ig.GammaR    0.1*ig.Alpha     0.1*ig.k_trap   0.1*ig.k_cyt0    0.*ig.k_cyt1    1e-1    0];
ub=[5000     10*ig.GammaR     10*ig.Alpha     10*ig.k_trap     10.*ig.k_cyt0    100*ig.k_cyt1    1e1     1.01];
...
% thi plots the initial fit in black
...
if multi == 1
    ...
    lb = [u0(1:2).*lbmult, 0.5, u0(4:6).*lbmult];
    ub = [u0(1:2).*ubmult, 1, u0(4:6).*ubmult];
...
if usefixnfit == 0 % will fit all parameters simultaneously (-Ora)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%% SECOND FIT %%%%%%%%%%%%%%%%%%%%%%%
    %%%%% [Beta     GammaR        Alpha          k_trap        k_cyt0        k_cyt1       mass       nu]
    lb2=  [0.1*u2i(1)      0.1*u2i(2)    0.90*u2i(3)    0.1*u2i(4)    0.1*u2i(5)    0.5*u2i(6)   1e-14    0.01];    
    ub2=  [10*u2i(1)    10*u2i(2)     1.1*u2i(3)     10*u2i(4)     10*u2i(5)     2*u2i(6)     1e2      1.01];
    W2=W;%.*C(:)';
    ...
    for fitNum = 1:length(lbscale)
        ...
        if groupem == 1
            ...
            %vary ktrap, kcyt0, kcyt1 
            lb = [u0(1:3).*lbfix, u0(4:6).*lbmult, 0, 0];
            ub = [u0(1:3).*ubfix, u0(4:6).*ubmult, 100, 100];
            else
            %vary beta, gamma, alpha 
            lb = [u0(1:3).*lbmult, u0(4:6).*lbfix, 0, 0];
            ub = [u0(1:3).*ubmult, u0(4:6).*ubfix, 100, 100];
            end
        elseif groupem == 2
            if mod(fitNum,2) == fitOrder
            %vary ktrap, beta
            lb = [u0(1).*lbmult, u0(2:3).*lbfix, u0(4).*lbmult, u0(5:6).*lbfix, 0, 0];
            ub = [u0(1).*ubmult, u0(2:3).*ubfix, u0(4).*ubmult, u0(5:6).*ubfix, 100, 100];
            else
        ...
        options_fit1 = optimset('TolFun',1e-9,'TolX',1e-9,'Display','iter','MaxFunEvals',1e5,'MaxIter',2e3);%,'DiffMinChange',1e-3);
        [paramsList(fitNum+1+multi,:), resNormList(fitNum+1+multi), resList(fitNum+1+multi,:)]=lsqcurvefit(@theor_trap_frf_r9,u0,xdata,ydata,lb,ub,options_fit1,abs(H(:))',PHfrf,W,Wphase,ydat10',fsamp,Rbead,Zbead,kT,falias); 
...
ww=2*pi.*FP;
ig.Gamma0 = 7.2e-06; % manual input, drag coefficient of water at 32 degrees [pN*s/nm]

g=k_cyt0 + k_cyt1.*((j.*ww).^alpha)./gamma(alpha)...
    + gammar*ig.Gamma0.*j.*ww;
Gp=g./(6*pi.*Rbead);
v=imag(g)./ww; %this gives viscous drag coefficient, not viscosity. units = pn *s /nm
...
savename=[matfilename,'OTC-nosqwv-',fitmethod,paramset,PSfit,fminsies,'.mat'];
save(savename)
